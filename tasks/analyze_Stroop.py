...
def analyze_stroop_results():
    ...
    # 找到两个session都有数据的被试
    common_participants = set(results_df[results_df['session'] == 1]['participant_id']) & \
    ...
    for pid in common_participants:
        s1_data = results_df[(results_df['session'] == 1) & (results_df['participant_id'] == pid)]
        s2_data = results_df[(results_df['session'] == 2) & (results_df['participant_id'] == pid)]
    ...
    # 正确率对比图
    sns.barplot(x='session', y='accuracy', data=results_df, ax=ax1,
    ...
    # 反应时对比图
    sns.barplot(x='session', y='rt', data=results_df, ax=ax2,
                palette=['lightblue', 'lightgreen'])
    ...
    plt.tight_layout()
    plt.savefig('stroop_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()
