...
def recalculate_stroop_stats(raw_data_path):
    ...
    def calculate_block_stats(block_df):
        ...
        congruent_trials = block_df[block_df.apply(lambda x: x['word'] == {
            'Red': '红',
            'Green': '绿',
            'Blue': '蓝'
        }[x['color']], axis=1)]
        ...
        incongruent_trials = block_df[~block_df.index.isin(congruent_trials.index) & 
...
stats_df = recalculate_stroop_stats('data/stroop_20_session2.csv')
...
for session in [1, 2]:
    print(f"\nSession {session} 统计:")
    session_stats = stats_df[(stats_df["level"] == "total") & 
                            (stats_df["session"] == session)]
    
    if not session_stats.empty:
        stats = session_stats.iloc[0]
        # Print key statistics if they exist in the stats DataFrame
