...
def analyze_nback_correct_rt():
    ...
    # 计算描述性统计
    desc_stats = (
        results_df.groupby(["session", "n_back"])
        .agg({"mean_rt": ["mean", "std", "count"]})
        .round(2)
    )
    ...
    for n in sorted(results_df["n_back"].unique()):
        # 获取完成了两个session的被试数据
        participants = set(
            results_df[(results_df["session"] == 1) & (results_df["n_back"] == n)][
                "participant_id"
            ]
        ) & set(
            results_df[(results_df["session"] == 2) & (results_df["n_back"] == n)][
                "participant_id"
            ]
        )
        ...
        if participants:  # 如果有完成两个session的被试
            ...
            for pid in participants:
                s1_rt = results_df[
                    (results_df["session"] == 1)
                    & (results_df["n_back"] == n)
                    & (results_df["participant_id"] == pid)
                ]["mean_rt"].iloc[0]
                s2_rt = results_df[
                    (results_df["session"] == 2)
                    & (results_df["n_back"] == n)
                    & (results_df["participant_id"] == pid)
                ]["mean_rt"].iloc[0]
    ...
    for i, session in enumerate([1, 2]):
        for j, n in enumerate(sorted(results_df["n_back"].unique())):
            mean_rt = results_df[
                (results_df["session"] == session) & (results_df["n_back"] == n)
            ]["mean_rt"].mean()
            x = j + (i - 0.5) * 0.35
